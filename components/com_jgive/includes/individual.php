 ...
 * @since  2.5.0
 */
class JGiveIndividual extends CMSObject
{
	/**
	 * The auto incremental primary key of the individual donor
	 *
	 * @var    integer
	 * @since  2.5.0
	 */
	 ...
	 * @since  2.5.0
	 */
	private static $individualObj = array();
	...
	/**
	 * Returns the global individual donor object
	 *
	 * @param   integer  $id  The primary key of the individual donor to load
	 *
	 * @return  JGiveIndividual  The donor object.
	 *
	 * @since   2.5.0
	 */
	public static function getInstance($id = 0)
	{
		if (!$id)
		{
			return new JGiveIndividual;
		}

		// Check if the donor id is already cached.
		if (empty(self::$individualObj[$id]))
		{
			self::$individualObj[$id] = new JGiveIndividual($id);
		}

		return self::$individualObj[$id];
	}
	 ...
	 *
	 * @return  integer|boolean  individual donor id on success
	 *
	 * @since   2.5.0
	 */
	public function addIndividualDonor($data)
	{
		...
		if ($isRecordExist == 0)
		{
			$app->enqueueMessage(Text::_('JLIB_DATABASE_ERROR_EMAIL_INUSE'), 'error');
		...
		if (array_key_exists('create_user', $data) && $data['create_user'] == 1)
		{
			$regdata               = array();
			$regdata['name']       = $data['first_name'] . ' ' . $data['last_name'];
			$regdata['user_name']  = $data['email'];
			$regdata['user_email'] = $data['email'];
			...
			if (empty($data['user_id']))
			{
				...
				JLoader::import('registration', JPATH_ADMINISTRATOR . '/components/com_jgive/models');
				$JgiveModeluserRegistration = new JgiveModeluserRegistration;

				// When email match with any users, set user_id of individual with user's id.
				if (!empty($isUserExist))
				{
					$data['user_id'] = $isUserExist;
					$JgiveModeluserRegistration->store($regdata);
				}

				// When email doesn't match with any users, create user and set its id in individual data.
				else
				{
					$user_id = $JgiveModeluserRegistration->store($regdata);
	...
	public function save()
	{
		...
		try
		{
			...
			Factory::getApplication()->triggerEvent('onContentAfterSave', array('com_jgive.individual', $table, $isNew, $this->getProperties()));
	...
	public function validateFormData($data)
	{
		...
		$form = $individualFormModel->getForm();

		// If allow create user then email field is required
		if ($data['create_user'])
		{
			$form->setFieldAttribute('email', 'required', 'true');
		}

		$formData = $individualFormModel->validate($form, $data);

		if ($formData === false)
		{
			// Handle how to handle the multiple errors in api
			$errors = $individualFormModel->getErrors();
	...
	/**
	 * Checking count of existing email in individual table
	 *
	...
	public function isExist($email, $vendorId)
	{
		...
		$individualTable->load(array('email' => $email, 'vendor_id' => (int) $vendorId));
		...
		if ($this->id == 0 || $this->id == "")
		{
			if (empty($individualTable->id))
			{
				...
				// When email not found in individuals table as well as users table.
				return 1;
			}
		}
		else
		{
			if ($individualTable->id == $this->id || $individualTable->id == 0)
			{
				return 1;
			}
		}
	...
	/**
	 * Method to get total donation amount donated by donor
	 *
	...
	public function getTotalDonatedAmount($formattedAmount = false)
	{
		...
		$orderModel->setState('filter.contributor_id', (int) $this->id);
		$orderModel->setState('filter.status', COM_JGIVE_CONSTANT_ORDER_STATUS_COMPLETED);
		$orderModel->setState('filter.donor_type', 'ind');
	...
	{
		$orderModel = JGive::model('orders', array('ignore_request' => true));
		$orderModel->setState('filter.contributor_id', $this->id);
		$orderModel->setState('filter.status', COM_JGIVE_CONSTANT_ORDER_STATUS_COMPLETED);
		$orderModel->setState('filter.donor_type', 'ind');
