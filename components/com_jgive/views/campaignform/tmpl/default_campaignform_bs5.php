...
<div class="col-xs-12 col-md-6">
	...
	<div class="row launchForm__date">
		<div class="col-xs-8 col-sm-6">
			<div class="form-group">
				<i class="fa fa-calendar af-pr-5 text-primary af-font-bold"
					aria-hidden="true"></i><?php echo $this->form->getLabel('start_date'); ?>
	...
	<div class="form-group">
		<?php echo $this->form->getLabel('lifetimecamp'); ?>
		<?php echo $this->form->getInput('lifetimecamp'); ?>
	</div>
	...
	<div class="row af-mt-10 launchForm__type">
		<div class="col-xs-12 col-sm-6">
			<div class="form-group">
				...
				<?php echo str_replace("custom-select", "form-select custom-select", $this->form->getInput('org_ind_type')); ?>
			</div>
		</div>
		<div class="col-xs-12 col-sm-6">
			<div class="form-group">
				...
				<?php echo str_replace("custom-select", "form-select custom-select", $this->form->getInput('category_id')); ?>
			</div>
		</div>
	</div>
	...
	<!--USD input-->
	<div class="row">
		<div class="col-xs-8 col-sm-6 col-md-6 launchForm__goal af-mt-10 bs5CampaignForm">
			<div class="form-group">
	...
	<div class="row af-mt-10">
		<div class="col-xs-12 col-sm-6">
			<div class="form-group">
				<label>
					<?php echo $this->form->getLabel('amount_suggestions'); ?>
				</label>
				<?php
				echo $this->form->getInput('amount_suggestions');
				?>
			</div>
		</div>
	</div>
...
<div class="col-xs-12 col-md-5 offset-md-1">
	<div class="row">
		...
		<div class="progress af-d-none" id="campaign-progress-bar">
			<div class="progress-bar progress-bar-striped active af-w-100" role="progressbar" aria-valuenow="100"
		...
		<div class="col-sm-4 col-md-6 col-xs-12">
			<div class="form-group jgive_campaign_upload">
				<input type="hidden" name="jform[image][new_image]" id="jform_campaign_image"
					value="<?php echo $mediaId; ?>" />
				<input type="hidden" name="jform[image][old_image]" id="jform_campaign_old_image" value="" />
				<img src="<?php echo $this->campaignImage; ?>" id="uploaded_media" class="img-responsive m-2">
			</div>
		</div>
...
{
    const form = jQuery('#adminForm');
    const endDateInput = jQuery('#jform_end_date');

    form.on('submit', function (event)
	{
        const endDateValue = new Date(endDateInput.val());
        const currentDate = new Date();
        // Check if the end date entered by the user is in the past
        if (endDateValue < currentDate)
		{
            event.preventDefault();
            alert('<?php echo Text::_('COM_JGIVE_END_DATE_ERROR')?>');
        }
    });
});
</script>