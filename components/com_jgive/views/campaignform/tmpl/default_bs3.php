...
if ($this->allowed == 1)
{
	if ($this->checkVendorApproval)
	{
			...
			if ($this->vendorProfileStatus < 100)
			{
				?>
					<div class="alert alert-warning">
						...
						<a href="<?php echo $this->editVendor;?>" target="_blank"><?php echo Text::_('COM_JGIVE_UPDATE_CAMP_SUCCESS_STATUS_TASK_1');?></a>
					</div>
				<?php
			}
			else
		...
		else
		{
			...
			Route::_('index.php?option=com_tjvendors&view=vendor&layout=edit&client=com_jgive&Itemid=' . $this->vendorProfileMenuId), strlen(Uri::base(true)) + 1);
			?>
			<div class="alert alert-warning">
				<?php echo Text::_('COM_JGIVE_USER_NOT_VENDOR');?>
				<a href="<?php echo $newVendorForm;?>" target="_blank"><?php echo Text::_('COM_JGIVE_UPDATE_CAMP_SUCCESS_STATUS_TASK_1');?></a>
			</div>
		<?php
		}
		?>
		<div id="jgiveWrapper" class="<?php echo COM_JGIVE_WRAPPAER_CLASS;?>">
			<form action="" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data" class="form-validate launchForm">
				<div class="row">
					...
					<?php echo $this->loadTemplate('campaignform_bs3');?>
				</div>
					...
					<div class="col-xs-12">
						<div class="af-my-20">
							<ul id="launchForm_nav" class="list-unstyled launchForm__nav nav nav-tabs af-d-flex">
								<?php
									...
									{
									?>
										<li class="launchForm__panel active">
										<a data-toggle="tab" href="#give-away" class="af-font-bold text-primary">
											<span>
								...
								<?php
									...
									{
									?>
										<li class="launchForm__panel">
										<a data-toggle="tab" href="#gallery" class="af-font-bold text-primary">
											<span>
								...
								<?php
									...
									if ($this->form_extra)
									{
										?>
										<li class="launchForm__panel">
											<a data-toggle="tab" href="#extraFields" class="af-font-bold text-primary">
												<span>
						...
						<div class="tab-content af-mb-15">
							...
							<?php
								...
								{
								?>
									<div id="give-away" class="tab-pane fade in active">
										<div class="row">
										<div class="col-xs-12 col-sm-6">
											<div>
											<?php 
											$this->form->setFieldAttribute('givebacks', 'layout', 'JGive.GiveBacksSubform.layouts.bs3.subform.repeatable');
											echo $this->form->getInput('givebacks'); ?>
											</div>
										</div>
										</div>
									</div>
								...
								if ($this->imageGallery || $this->videoGallery)
								{
									...
									<div id="gallery" class="tab-pane fade">
										<div class="row">
											<div class="col-xs-12">
												<div class="alert alert-info">
													<span><?php echo $this->form->getInput('video_on_details_page');?> &nbsp;&nbsp;</span>
										...
										<?php
											...
											?>
												<div class="col-xs-12 col-md-6 af-mb-10">
													...
													<div class="form-group col-xs-6">
														<br>
														<input type="button" class="validate_video_link btn btn-primary btn-success btn-donate af-mt-5" onclick="tjMediaFile.validateFile(this,1, <?php echo $this->isAdmin;?>)"
											...
											<div class="col-xs-12">
												<div class="subform-wrapper">
													<div id="gallery_media" class="media_gallary_parent row">
														<div class="clone_media hide_jgdiv col-xs-6 col-sm-4 col-md-3 af-my-15 hoverEffect">
															<div class="overlay">
																<button class="btn p-0" id="delete_media" onclick="tjMediaFile.tjMediaGallery.deleteMedia(this, <?php echo $this->isAdmin;?>, '<?php echo $token;?>', 'com_jgive.campaign', '<?php echo $this->item->id?>');return false;">
									...
									<div id="extraFields" class="tab-pane fade">
										<div class="row">
											...
											<?php
												...
												{
													echo $this->loadTemplate('extrafields_bs3');
												}
												?>
											</div>
										</div>
				...
				<div class="row  af-mt-20">
					<div class="col-xs-12 form-group">
						<button type="button" class="btn btn-primary" onclick="jgiveCommon.campaignForm.save('campaignform.save')">
							<span>
								<?php echo Text::_('JSUBMIT'); ?>
							</span>
						</button>

						<button type="button" class="btn btn-default" onclick="Joomla.submitbutton('campaignform.cancel')">
