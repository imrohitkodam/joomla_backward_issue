...
<div id="jgiveWrapper" class="<?php echo COM_JGIVE_WRAPPAER_CLASS;?> container">
					...
					?>
						<div class="campaignImg">
							...
							if ($this->cdata['campaign']->video_on_details_page == 1 && !empty ($this->cdata['campaign']->videoPlgParams))
							{
								...
								{
									$videoUrl = $videoPlgParams['file'];
									$separator = (strpos($videoUrl, '?') !== false) ? '&' : '?';
									$finalUrl = $videoUrl . $separator . "autoplay=1&mute=1";
									?>
									<iframe width="100%" height="320" src="<?php echo $finalUrl; ?>" allow="autoplay"></iframe>
								<?php
								}
								...
								{
								?>
									<video width="100%" height="320" controls autoplay muted>
					...
					<div class="modal-dialog">
						...
						<div class="modal-content">
							...
							<div class="modal-footer">
								<button type="button" id="campaignCloseModalbtn" class="btn btn-default" data-bs-dismiss="modal">
	...
	<!--Tab -->
	<div class="row1">
		<div class="ftab col-md-12">
			<div class="row1 campaignTab border-b af-pt-10">
				<div class="col-xs-12 tab">
					<ul id="myTab" class="nav nav-pills text-uppercase justify-content-start gap-2 campaignTab__ul launchForm__nav af-d-flex">
						<?php
						if ($this->cdata['params']['campaign_activity'] == 1 || !isset($this->cdata['params']['campaign_activity'])):?>
						<li class="nav-item">
							<a data-bs-toggle="tab" data-bs-target="#camp_activity" class="nav-link active px-4 py-2 fw-bold text-primary border border-primary hover-effect">
						...
						?>
						<li class="nav-item">
							<a data-bs-toggle="tab" data-bs-target="#camp_donors" class="nav-link px-4 py-2 text-primary border border-primary hover-effect fw-bold <?php echo $this->cdata['params']['campaign_activity'] == 0 ? 'active' : ''?> af-pb-10 af-font-bold">
								<?php echo ($this->cdata['campaign']->type == 'donation') ? strtoupper(Text::_('COM_JGIVE_DONORS')) : strtoupper(Text::_('COM_JGIVE_INVESTORS'));?>
								<span class="badge bg-secondary ms-2"><?php echo $this->cdata['campaign']->campaignDonorsCount; ?></span>
							</a>
						</li>
						<?php
						}
						...
						?>
							<li class="nav-item">
								<a data-bs-toggle="tab" data-bs-target="#camp_reports" class="nav-link px-4 py-2 fw-bold border border-primary text-primary hover-effect">
						...
						if (isset($this->item['campaign']->gallery))
						{
							if (!empty($this->cdata['image']) && $this->params->get('img_gallery'))
							{
								?>
									<li class="nav-item">
										<a data-bs-toggle="tab" data-bs-target="#camp_gallery" class="nav-link px-4 py-2 fw-bold border border-primary text-primary hover-effect">
						...
						?>
							<li class="nav-item">
								<a data-bs-toggle="tab" data-bs-target="#additional_info" class="nav-link px-4 py-2 fw-bold border border-primary text-primary hover-effect">
									<?php echo strtoupper(Text::_('COM_JGIVE_EXTA_FIELDS'));?>
								</a>
							</li>
						<?php
						}
							// Show the tab only if beneficiary stories array having data
						if (((!in_array('beneficiary_stories', $this->hideFields) && $this->hideShowFields == 1) || $this->hideShowFields == 0) && (!empty($this->item['campaign']->beneficiaryStories)))
						{
							?>
							<li class="nav-item">
								<a data-bs-toggle="tab" data-bs-target="#camp_beneficiary_stories" class="nav-link px-4 py-2 fw-bold border border-primary text-primary hover-effect">
									<?php echo strtoupper(Text::_('COM_JGIVE_BENEFICIARY_STORIES')); ?>
								</a>
							</li>
							<?php
						}
						...
						if ((!in_array('give_back', $this->hideFields) && $this->hideShowFields == 1) || $this->hideShowFields == 0)
						{
							if (count($this->cdata['givebacks']) >= 1)
							{
								?>
									<li class="nav-item">
										<a data-bs-toggle="tab" data-bs-target="#camp_giveback_tab" class="nav-link px-4 py-2 fw-bold border border-primary text-primary hover-effect">
											<?php echo strtoupper(Text::_('COM_JGIVE_SINGLE_GIVEBACK')); ?>
										</a>
									</li>
								<?php
							}
				...
				<div class="col-xs-12 af-mt-20">
					...
					<div class="tab-content">
						...
						<div id="camp_gallery" class="tab-pane af-mt-20">
							<?php
								echo $this->loadTemplate("gallary_bs5");
							?>
						</div>

						<!---Tab-BENEFICIARY STORIES---->
						<div id="camp_beneficiary_stories" class="tab-pane af-mt-20">
							<?php
								echo $this->loadTemplate("beneficiary_stories_bs5");
							?>
						</div>

						<div id="camp_giveback_tab" class="tab-pane af-mt-20">
							<?php
								if ((!in_array('give_back', $this->hideFields) && $this->hideShowFields == 1) || $this->hideShowFields == 0)
								{
									if (count($this->cdata['givebacks']) >= 1)
									{
									?>
										<div class="col-xs-12 offset-md-1 af-pt-25 af-text-md-left center">
											<?php
											$i = 0;
...
<script type="text/javascript">
	...
	{
		let video = document.querySelector(".campaignImg video");
		if (video) {
