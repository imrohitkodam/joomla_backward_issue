...
class TjStrapper
{
	...
	public static $com_tjreports_installed       = 0;

	public static $com_tmt_installed             = 0;

	public static $com_people_suggest_installed  = 0;

	public static $com_tjvendors_installed       = 0;
	...
	public static $load_com_tjreports_assets       = 0;

	public static $load_com_tmt_assets             = 0;

	public static $load_com_people_suggest_assets  = 0;

	public static $load_com_tjvendors_assets        = 0;
	...
	public static function initializeVars()
	{
		...
		$plugin = PluginHelper::getPlugin('system', 'tjassetsloader');
		...
		if ($plugin)
		{
			...
			// We dont need to load JS in header in Joomla 4 as we have asset manager
			if (JVERSION >= '4.0.0')
			{
				self::$fix_js = 0;
			}
		...
		if (self::$load_com_emailbeautifier_assets)
		{
			...
			if (File::exists(JPATH_ROOT . '/components/com_emailbeautifier/emailbeautifier.php'))
			{
				if (ComponentHelper::isEnabled('com_emailbeautifier', true))
				{
		...
		if (self::$load_com_jbolo_assets)
		{
			...
			if (File::exists(JPATH_ROOT . '/components/com_jbolo/jbolo.php'))
			{
				if (ComponentHelper::isEnabled('com_jbolo', true))
				{
		...
		if (self::$load_com_jgive_assets)
		{
			...
			if (File::exists(JPATH_ROOT . '/components/com_jgive/jgive.php'))
			{
		...
		if (self::$load_com_jmailalerts_assets)
		{
			...
			if (File::exists(JPATH_ROOT . '/components/com_jmailalerts/jmailalerts.php'))
			{
				if (ComponentHelper::isEnabled('com_jmailalerts', true))
				{
		...
		if (self::$load_com_quick2cart_assets)
		{
			...
			if (File::exists(JPATH_ROOT . '/components/com_quick2cart/quick2cart.php'))
			{
		...
		if (self::$load_com_socialads_assets)
		{
			...
			if (File::exists(JPATH_ROOT . '/components/com_socialads/socialads.php'))
			{
				if (ComponentHelper::isEnabled('com_socialads', true))
				{
		...
		if (self::$load_com_tjfields_assets)
		{
			...
			if (File::exists(JPATH_ROOT . '/components/com_tjfields/tjfields.php'))
			{
				if (ComponentHelper::isEnabled('com_tjfields', true))
				{
		...
		if (self::$load_com_tjlms_assets)
		{
			...
			if (File::exists(JPATH_ROOT . '/components/com_tjlms/tjlms.php'))
			{
		...
		if (self::$load_com_tjreports_assets)
		{
			// Check if com_tjreports is installed
			if (File::exists(JPATH_ROOT . '/components/com_tjreports/tjreports.php'))
			{
				if (ComponentHelper::isEnabled('com_tjreports', true))
				{
					self::$com_tjreports_installed = 1;
					self::$tj_extensions[]     = 'com_tjreports';
				}
			}
		}
		...
		if (self::$load_com_tmt_assets)
		{
			...
			if (File::exists(JPATH_ROOT . '/components/com_tmt/tmt.php'))
			{
				if (ComponentHelper::isEnabled('com_tmt', true))
				{
		...
		{
			// Check if com_tmt is installed
			if (File::exists(JPATH_ROOT . '/components/com_psuggest/psuggest.php'))
			{
				if (ComponentHelper::isEnabled('com_psuggest', true))
				{
		...
		if (self::$load_com_tjvendors_assets)
		{
			// Check if com_tmt is installed
			if (File::exists(JPATH_ROOT . '/components/com_tjvendors/tjvendors.php'))
			{
				if (ComponentHelper::isEnabled('com_tjvendors', true))
				{
					self::$com_tjvendors_installed = 1;
					self::$tj_extensions[]   = 'com_tjvendors';
				}
			}
		}
	}
		...
		else
		{
			if (self::$load_com_invitex_assets)
			{
				if (self::$com_invitex_installed == 1)
				{
					...
					{
						$cominvitexHelper->getInvitexJsFiles($jsFilesArray, self::$firstThingsScriptDeclaration);
					}
				}
			}
			...
			if (self::$load_com_jticketing_assets)
			{
				if (self::$com_jticketing_installed == 1)
				{
					...
					$path      = JPATH_SITE . "/components/com_jticketing/includes/utilities.php";
					$utilities = self::TjloadClass($path, 'jticketingutilities');

					if (method_exists($utilities, "getJticketingJsFiles"))
					{
						// Add component specific js file in provided array.
						$utilities->getJticketingJsFiles($jsFilesArray);
					}
				}
			}
			...
			if (self::$load_com_socialads_assets)
			{
				if (self::$com_socialads_installed == 1)
				{
					...
					$xml             = simplexml_load_file(JPATH_SITE . '/administrator/components/com_socialads/socialads.xml');
	...
	public static function remove_duplicate_files($assetsarray)
	{
		...
		foreach ($assetsarray as $key => $file)
		{
			if ($file[0] == '/')
			{
				$assets_name_relative = Uri::root(true) . $file;
				$assets_name_absolute = Uri::root() . $file;
			}
			else
			{
				$assets_name_relative = Uri::root(true) . '/' . $file;
	 ...
	 *
	 * @param   array  &$scriptList  Array of css files
	 * @param   array  $filenames    Array of css files
	 *
	 * @return  void
	 *
	 * @since   3.0
	...
	public static function getCSSscripts(&$scriptList, $filenames)
	{
		...
		if ($comb_mini && is_writable($cssfile_path))
		{
			...
			if (File::write($cssfile_path, $data))
			{
				$scriptList[] = '<link rel="stylesheet" href="' . Uri::root(true) . '/components/com_jbolo/css/jbolocss.php" type="text/css" />';
			}
			else
			{
				foreach ($filenames as $file)
				{
					...
					$scriptList[] = '<link rel="stylesheet" href="' . Uri::root(true) . '/' . $file . '" type="text/css" />';
				}
			}
		}
		else
	 ...
	 *
	 * @param   array  &$scriptList  Array of js files
	 * @param   array  $filenames    Array of js files
	 *
	 * @return  void
	 *
	 * @since   3.0
	...
	public static function getJSscripts(&$scriptList, $filenames)
	{
		...
		if ($comb_mini && is_writable($jsfile_path) && self::$com_jbolo_installed)
		{
			...
			if (File::write($jsfile_path, $data))
			{
				$scriptList[] = '<script type="text/javascript" src="' . Uri::root(true) . '/components/com_jbolo/js/jbolojs.php"> </script>';
			}
			else
			{
				foreach ($filenames as $file)
				{
					if ($file[0] == '/')
					{
						$scriptList[] = '<script src="' . Uri::root(true) . '/' . $file . '" type="text/javascript"></script>';
					}
					else
					{
						$scriptList[] = '<script src="' . Uri::root(true) . '/' . $file . '" type="text/javascript"></script>';
	...
	public static function loadTjAssets($extension = '')
	{
		if ($extension != '')
		{
			switch ($extension)
			{
				...
				case 'com_tjreports':
				self::$load_com_tjreports_assets = 1;
				break;
				...
				case 'com_tjvendors':
				self::$load_com_tjvendors_assets = 1;
				break;
...
function onAfterRenderTj()
{
	...
	$jbolo_dynamic_js = $jsChatTemplates = '';
	$cssScripts = $jsScripts1 = $jsScripts2 = array();
	...
	if (TjStrapper::$fix_js == 1)
	{
		...
		{
			$includescripts  = $scriptHtml . $includescripts;
			$body = TjStrapper::str_replace_first('<head>', '<head>' . $includescripts, $body);
		}
		else
		{
			$body = TjStrapper::str_replace_first('<head>', '<head>' . $scriptHtml, $body);
