		...
		if(window.File && window.FileReader && window.FileList && window.Blob) {
			...
			if (allowedMimeTypesArray !== undefined && (allowedMimeTypesArray.length > 1)) {
				...
				if(jQuery.inArray(ftype, allowedMimeTypesArray) == -1) {
					alert(Joomla.Text._('LIB_TECHJOOMLA_ERR_MSG_FILE_ALLOW'))

					return Joomla.Text._('LIB_TECHJOOMLA_ERR_MSG_FILE_ALLOW');
				}
			}
			...
			if(fsize > allowedMediaSizeInKb) {
				var tjFileSizeErrMsg = Joomla.Text._('LIB_TECHJOOMLA_ALLOWED_FILE_SIZE')
