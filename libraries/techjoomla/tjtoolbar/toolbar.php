<?php
/**
 * @package     Techjoomla.Libraries
 * @subpackage  Ttoolbar
 *
 * @author      Techjoomla <extensions@techjoomla.com>
 * @copyright   Copyright (C) 2009 - 2019 Techjoomla. All rights reserved.
 * @license     http://www.gnu.org/licenses/gpl-2.0.html GNU/GPL
 */

defined('_JEXEC') or die;

use Joomla\Utilities\ArrayHelper;
use Joomla\CMS\Language\Text;
use Joomla\CMS\Toolbar\Toolbar;

define('TTOOLBAR_ICON_ADDNEW', " icon-plus-2");
define('TTOOLBAR_ICON_EDIT', " icon-apply");
define('TTOOLBAR_ICON_DELETE', " icon-trash");
define('TTOOLBAR_ICON_PUBLISH', " icon-checkmark");
define('TTOOLBAR_ICON_UNPUBLISH', " icon-unpublish");

/**
 * Toolbar handler
 *
 * @package     Joomla.Libraries
 * @subpackage  Toolbar
 * @since       1.5
 */
class TToolbar extends Toolbar
{
	/**
	 * Toolbar cssClass
	 *
	 * @var    string
	 */
	protected $toolbarPositionClass = array();

	/**
	 * Constructor
	 *
	 * @param   string  $name                  The toolbar name.
	 * @param   string  $toolbarPositionClass  Toolbar position class.
	 *
	 * @since   1.0
	 */
	public function __construct($name = 'toolbar', $toolbarPositionClass = '')
	{
		$this->_name = $name;
		$this->toolbarPositionClass = $toolbarPositionClass;

		// Set base path to find buttons.
		$this->_buttonPath[] = __DIR__ . '/button';
	}

	/**
	 * Returns the global TToolbar object, only creating it if it
	 * doesn't already exist.
	 *
	 * @param   string  $name      The name of the toolbar.
	 * @param   string  $cssClass  pass emty value.
	 *
	 * @return  TToolbar  The TToolbar object.
	 *
	 * @since   1.0
	 */
	public static function getInstance($name = 'toolbar', $cssClass = '')
	{
		if (empty(self::$instances[$name]))
		{
			self::$instances[$name] = new TToolbar($name, $cssClass);
		}

		return self::$instances[$name];
	}

	/**
	 * Render a tool bar.
	 *
	 * @return  string  HTML for the toolbar.
	 *
	 * @since   1.0
	 */
	public function render(array $options = [])
	{
		$html = array();

		$html[] = '
		<style>
			.btn-toolbar .btn-wrapper {
				display: inline-block;
				margin: 0 0 5px 5px;
			}
		</style>';

		$html[] = '<div class="row-fluid">';
			$html[] = '<div class="span12">';
				$html[] = '<div class="btn-toolbar ' . $this->toolbarPositionClass . '" id="' . $this->_name . '">';

					// Render each button in the toolbar.
					foreach ($this->_bar as $button)
					{
						$html[] = $this->renderButton($button);
					}

				$html[] = '</div>';
			$html[] = '</div>';
		$html[] = '</div>';

		return implode('', $html);
	}

	/**
	 * Render a button.
	 *
	 * @param   object  &$node  A toolbar node.
	 *
	 * @return  string
	 *
	 * @since   1.0
	 */
	public function renderButton(&$node)
	{
		$taskName = $node[0];
		$text = $node[1];
		$class = $node[2];
		$attributs = $node[3];

		// Attributes and these value are converted into a string
		if (is_array($attributs))
		{
			$attributs = ArrayHelper::toString($attributs);
		}

		// Add button 'onclick' Javascript
		$spiltTask = explode('.', $taskName);

		Text::script('TTOOLBAR_NO_SELECT_MSG');
		$task = "Joomla.submitbutton('" . $taskName . "');";
		$alert = "alert(Joomla.Text._('TTOOLBAR_NO_SELECT_MSG'));";
		$task = "onclick = if(document.adminForm.boxchecked.value==0){" . $alert . "}else{" . $task . "}";

		switch ($spiltTask[1])
		{
			default:

			case 'addNew':
				$taskName = "&apos;" . $taskName . "&apos;";
				$task = 'onclick="Joomla.submitbutton(' . $taskName . ')"';

				if (empty($class))
				{
					$class = TTOOLBAR_ICON_ADDNEW;
				}
			break;

			case 'edit':

				if (empty($class))
				{
					$class = TTOOLBAR_ICON_EDIT;
				}
			break;

			case 'publish':

				if (empty($class))
				{
					$class = TTOOLBAR_ICON_PUBLISH;
				}
			break;

			case 'unpublish':

				if (empty($class))
				{
					$class = TTOOLBAR_ICON_UNPUBLISH;
				}
			break;

			case 'delete':

				if (empty($class))
				{
					$class = TTOOLBAR_ICON_DELETE;
				}
			break;

			case 'custom':
				$task = '';

				if (empty($class))
				{
					$class = TTOOLBAR_ICON_ADDNEW;
				}
			break;
		}

		// Apply Text
		$text = Text::_($text);

		// Generate button HTML
		$btnHtml = '<div class="btn-wrapper" id="toolbar-' . $spiltTask[1] . '">
						<button type="button" ' . $task . ' ' . $attributs . '>
							<span class="' . trim($class) . '"></span> ' . $text . '
						</button>
					</div>';

		return $btnHtml;
	}
}
