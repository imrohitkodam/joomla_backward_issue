 ...
 */
class TjControllerForm extends FormController
{
	/**
	...
	public function save($key = null, $urlVar = null)
	{
		...
		$recordId = $this->getInput()->getInt($urlVar);
		...
		if (!$this->allowSave($data, $key))
		{
			if ($isAjaxRequest)
			{
				...
				$returnData['msg']    = Text::_('JLIB_APPLICATION_ERROR_SAVE_NOT_PERMITTED') . ': ' . $this->getError();
			}
			else
			{
		...
		if ($validData === false)
		{
			...
			$returnData['status'] = 'error';
			$returnData['msg']    = Text::_('Error saving details');
			$msg = '';
			...
			for ($i = 0, $n = count($errors); $i < $n && $i < 3; $i++)
			{
				if ($errors[$i] instanceof Exception)
				{
					if ($isAjaxRequest)
					{
						$msg .= $msg . ' ' . Text::_($errors[$i]->getMessage());
					}
					else
					{
		...
		if ($return['error'] == 1)
		{
			...
			{
				$returnData['status'] = 'error';
				$returnData['msg'] = Text::sprintf('JLIB_APPLICATION_ERROR_SAVE_FAILED', $return['errorMsg']);
