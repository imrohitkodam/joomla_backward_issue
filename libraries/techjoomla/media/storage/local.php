 ...
 * @since  1.0.0
 */
class TJMediaStorageLocal extends CMSObject implements TjMedia
{
	// Media table id
	public $id = 0;
	...
	public function __construct($configs = array())
	{
		...
		else
		{
			...
			{
				$this->allowedExtension = explode(',', $param->get('restrict_uploads_extensions'));
			}
		}
	...
	public function upload($files = array())
	{
		foreach ($files as $file)
		{
			...
			$first = current($temp);
			$this->source = '';
			$this->source = round(microtime(true)) . "_" . UserHelper::genRandomPassword(5) . "_" . $first . '.' . $fileDetails['extension'];
	...
	public function delete()
	{
		...
		$mediaFilesTable = Table::getInstance('Files', 'TJMediaTable');
