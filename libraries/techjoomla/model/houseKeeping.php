...
class TjModelHouseKeeping
{
	...
	public function getHouseKeepingFromVersion($clientExtension)
	{
		...
		$query->select('MIN(' . $this->db->quoteName('id') . ')');
		$query->select($this->db->quoteName('version'));
		$query->from($this->db->quoteName('#__tj_houseKeeping'));
		$query->where($this->db->quoteName('client') . ' = ' . $this->db->quote($clientExtension));
		$query->where($this->db->quoteName('status') . ' = 0');
		$query->group($this->db->quoteName('version'));
	...
	protected function scriptExecuted($script)
	{
		...
		$query->select('*');
		$query->from($this->db->quoteName('#__tj_houseKeeping'));
		$query->where($this->db->quoteName('client') . ' = ' . $this->db->quote($client));
		$query->where($this->db->quoteName('version') . ' = ' . $this->db->quote($version));
	...
	protected function updateHouseKeepingStatus($client, $version, $title, $status = 0)
	{
		...
		$record = $this->db->loadObject();

		$record = empty($record) ? new stdClass : $record;
		...
		if (empty($record->id))
		{
			...
			$record->title = $title;
			$this->db->insertObject('#__tj_houseKeeping', $record, 'id');
			$result = $this->db->insertid();
		}
		else
		{
			$this->db->updateObject('#__tj_houseKeeping', $record, 'id');
			$result = $record->id;
		}
