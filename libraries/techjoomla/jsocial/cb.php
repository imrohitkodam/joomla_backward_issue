...
class JSocialCB implements JSocial
{
	...
	public function getProfileUrl(User $user, $relative = false)
	{
		...
		else
		{
			...
			$link = Route::_('index.php?option=com_comprofiler&task=userprofile&user=' . $user->id . '&Itemid=' . $itemid);

			if (strpos($link, Uri::root()) === false)
			{
	...
	public function getAvatar(User $user, $gravatar_size = '', $relative = false)
	{
		...
		$q = "SELECT a.id,a.username,a.name, b.avatar, b.avatarapproved
            ...
            WHERE a.id=b.user_id AND a.id=" . (int) $user->id;
		$db->setQuery($q);
	...
	public function addFriend(User $connect_from_user, User $connect_to_user)
	{
		...
		$sql			= "SELECT * FROM #__comprofiler_members
							WHERE referenceid=" . (int) $connect_from_user->id . " AND memberid  = " . (int) $connect_to_user->id;
		$db->setQuery($sql);
	...
	public function pushActivity($actor_id, $act_type, $act_subtype = '', $act_description = '', $act_link = '', $act_title='', $act_access='')
	{
		...
		{
			if (!file_exists($app->getCfg('absolute_path') . '/administrator/components/com_comprofiler/plugin.foundation.php'))
			{
				echo 'CB not installed!';
	...
	{
		return Folder::exists(JPATH_SITE . '/components/com_comprofiler');
	}
