...
class JSocialAlphauserpoints implements JSocial
{
	...
	{
		if (File::exists(JPATH_SITE . '/components/com_alphauserpoints/helper.php'))
		{
			require_once JPATH_SITE . '/components/com_alphauserpoints/helper.php';
		}

		if (File::exists(JPATH_SITE . '/components/com_altauserpoints/helper.php'))
		{
	 ...
	 * @since  1.0
	 */
	public function addStream(User $user, $options)
	{
	}
	 ...
	 * @since   1.0
	 */
	public function setStatus(User $user, $status, $options=array())
	{
	}
	...
	{
		return Route::_('index.php?option=com_users&view=registration&Itemid=' . UsersHelperRoute::getRegistrationRoute());
	}
	 ...
	 * @since  1.0
	 */
	public function sendMessage(User $user, $recepient)
	{
	}
	...
	public function checkExists()
	{
		...
		elseif(File::exists(JPATH_SITE . '/components/com_altauserpoints/helper.php'))
		{
			return true;
		}
		else
	 ...
	 * @since   1.0
	 */
	public function getAnyUserReferreID(User $user)
	{
		if (File::exists(JPATH_SITE . '/components/com_alphauserpoints/helper.php'))
		{
	 ...
	 */
	public function addpoints(User $user, $options = array())
	{
		...
		elseif (File::exists(JPATH_SITE . '/components/com_alphauserpoints/helper.php'))
		{
	 ...
	 * @since  1.0
	 */
	public function sendNotification(User $sender, User $receiver, $content = "JS Notification", $options = array(), $emailOptions = false)
	{
	}
	 ...
	 * @since   1.0
	 */
	public function addMemberToGroup($groupId, User $groupmember)
	{
	}
