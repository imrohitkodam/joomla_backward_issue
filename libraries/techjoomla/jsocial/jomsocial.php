...
class JSocialJomsocial implements JSocial
{
	...
	{
		return File::exists(JPATH_SITE . '/components/com_community/libraries/core.php');
	}
	...
	public function getProfileUrl(User $user, $relative = false)
	{
		...
		{
			$link = Uri::root() . 'index.php?option=com_community&view=profile&userid=' . $user->id;
			$link = CRoute::_($link);
		}
	...
	public function getAvatar(User $user, $gravatar_size = '', $relative = false)
	{
		...
		if (!$this->mainframe->isClient('site'))
		{
		...
		if ($ifurl === false)
		{
			...
			$uimage = str_replace(Uri::root(true) . '/', '', $uimage);
			$uimage = Uri::root() . $uimage;
		}
	...
	public function addFriend(User $connect_from_user, User $connect_to_user)
	{
		...
		$sql = "SELECT * FROM #__community_connection WHERE connect_from=" . (int) $connect_to_user->id . " AND connect_to = " . (int) $connect_from_user->id;
		$db->setQuery($sql);
