...
class JSocialEasysocial implements JSocial
{
	 ...
	 * @since   1.0
	 */
	public function getProfileUrl(User $user, $relative = false)
	{
		...
		if ($relative)
		{
			$link = str_replace(Uri::root(), '', $link);
		}
		else
		{
			$link = Route::_($link);
	 ...
	 * @since   1.0
	 */
	public function getAvatar(User $user, $gravatar_size = '', $relative = false)
	{
		$user   = ES::user($user->id);
		...
		if ($relative)
		{
			$uimage = str_replace(Uri::root(), '', $uimage);
		}
	 ...
	 *
	 * @return  StdClass[] friends object
	 *
	 * @since   1.0
	 */
	public function getFriends(User $user, $accepted=true, $options = array())
	{
	 ...
	 * @since   1.0
	 */
	public function addFriend(User $connect_from_user, User $connect_to_user)
	{
		$db = Factory::getContainer()->get('DatabaseDriver');
		$sql = "SELECT * FROM #__social_friends
				WHERE (actor_id=" . (int) $connect_from_user->id . " AND target_id  = " . (int) $connect_to_user->id . ")
				OR (target_id=" . (int) $connect_from_user->id . " AND actor_id  = " . (int) $connect_to_user->id . ")";
		$db->setQuery($sql);
		...
		if (!$once_done)
		{
			...
			$db->insertObject('#__social_friends', $insertfrnd, 'id');
		}
	}
	...
	public function getToolbar()
	{
		...
		JLoader::import('components.com_easysocial.includes.responsive.responsive', JPATH_ADMINISTRATOR);
		$TechjoomlaCommon = new TechjoomlaCommon;
		$socialResponsive = new SocialResponsive;

		$class = ($socialResponsive->isMobile())?'is-mobile':'';

		$easySocialVersion = $TechjoomlaCommon->getExtensionVersion('component', 'com_easysocial');

		if (version_compare($easySocialVersion, '4.0.11', '>='))
		{
			$document = Factory::getDocument();
			$renderer = $document->loadRenderer('module');
			$modules  = ModuleHelper::getModules('stackideas-toolbar');
		...
		elseif (version_compare($easySocialVersion, '2.0.0', 'ge') && version_compare($easySocialVersion, '4.0.11', '<') == 1)
		{
			return '<div id="es" class="es-frontend es-main  view-profile ' . $class . '" data-es-structure="">' . FD::toolbar()->render() . '</div>';
		}
		else
		{
	...
	{
		return File::exists(JPATH_ROOT . '/administrator/components/com_easysocial/includes/easysocial.php');
	}
	 ...
	 */
	public function addpoints(User $receiver, $options = array())
	{
		$return = array();
		...
		$return['success'] = ES::points()->assign($options['command'], $options['extension'], $receiver->id);
	}
	 ...
	 * @since  1.0.0
	 */
	public function addbadges(User $receiver, $options = array())
	{
		if (empty($options['command']) or empty($options['extension']))
		{
				$return['success'] = 0;
		...
		$return['success'] = ES::badges()->log($options['extension'], $options['command'], $receiver->id, '');
	}
	 ...
	 * @since  1.0
	 */
	public function sendNotification(User $sender, User $receiver, $content = "ES Notification", $options = array(), $emailOptions = false)
	{
		...
		$myUser = ES::user($receiver->id);

		$cmd = isset($systemOptions['cmd']) ? $systemOptions['cmd'] : '';
		//unset($systemOptions['cmd']);

		if (!isset($systemOptions['url']))
		{
			$systemOptions['url'] = Route::_($myUser->getPermalink());
		}

		return ES::notify($cmd, $recipient, $emailOptions, $systemOptions);
	}
	 ...
	 * @since   1.0
	 */
	public function addStream(User $options)
	{
	}
	 ...
	 * @since   1.0
	 */
	public function setStatus(User $user, $status, $options)
	{
	}
	...
	public function createGroup($data, $options = array())
	{
		...
		$esGroup->alias             = OutputFilter::stringURLSafe($data['title']);
		$esGroup->state             = 1;
		$esGroup->created           = Factory::getDate()->toSQL();
		$esGroup->params            = null;
		$esGroup->hits              = 0;
		...
		$esGroup->key               = md5(ES::date()->toSql() . $creator_uid->password . uniqid());
	 ...
	 *
	 * @return  false|true could be false, could be true
	 *
	 * @since   1.0
	 */
	public function addMemberToGroup($groupId, User $groupmember, $state = 1)
	{
		Table::addIncludePath(JPATH_ROOT . '/administrator/components/com_easysocial/tables');

		$db = Factory::getContainer()->get('DatabaseDriver');

		$table = Table::getInstance('clusternode', 'SocialTable', array('dbo', $db));
