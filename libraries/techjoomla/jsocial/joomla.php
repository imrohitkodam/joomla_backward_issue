...
class JSocialJoomla implements JSocial
{
	 ...
	 * @since   1.0
	 */
	public function getAvatar (User $user, $gravatar_size = 50, $relative = false)
	{
		$email = $user->email;
		$url = ($this->gravatar_secure) ? $this->gravatar_surl : $this->gravatar_url;
		$url .= ($email ? md5($email) : '') . '?d=' . $this->gravatar_default . '&rating=' . $this->gravatar_rating . '&s=' . $gravatar_size;
	...
	public function getProfileUrl(User $user, $relative = false)
	{
		...
		{
			$link = Route::_('index.php?option=com_users&view=profile&userid=' . $user->id . '&Itemid=' . $itemid);
