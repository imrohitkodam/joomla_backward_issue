...
?>
<form action="index.php?option=com_jgive&view=individuals" name="adminForm" id="adminForm" class="form-validate" method="post">
	...
	<table class='table'>
		<thead>
			<tr>
				...
				<th width="15%">
					<?php echo HTMLHelper::_('grid.sort', 'COM_JGIVE_INDIVIDUALS_NAME', 'first_name', $listDirn, $listOrder);?>
				</th>
				<th width="15%">
					<?php echo HTMLHelper::_('grid.sort', 'COM_JGIVE_INDIVIDUALS_VENDOR_NAME', 'vendor_id', $listDirn, $listOrder);?>
				</th>
				<th width="20%">
					<?php echo HTMLHelper::_('grid.sort', 'COM_JGIVE_INDIVIDUAL_EMAIL', 'email', $listDirn, $listOrder);?>
				</th>
				<th width="15%">
				...
				?>
				<th width="5%">
					<?php echo HTMLHelper::_('grid.sort', 'COM_JGIVE_INDIVIDUALS_ID', 'id', $listDirn, $listOrder)?>
				</th>
			</tr>
		</thead>
		<tbody>
		<?php
			if (!empty($this->items))
			{
				...
				{
					$link = Route::_('index.php?option=com_jgive&task=individual.edit&id=' . $row->id);
				?>
					<tr>
						<td>
						...
						<td>
							<a href="<?php echo $link; ?>">
								<?php echo htmlspecialchars($row->first_name ? $row->first_name : '',  ENT_COMPAT, 'UTF-8'); ?>
								<?php echo htmlspecialchars($row->last_name ? $row->last_name : '',  ENT_COMPAT, 'UTF-8'); ?>
							</a>
						</td>
						<td>
						...
						<td>
							<a
								href="<?php echo $link; ?>"
								title="<?php echo Text::_('COM_JGIVE_EDIT_INDIVIDUAL'); ?>">
...
Joomla.submitbutton = function(action)
{
	...
	// Show confirmation message before delete individual
	if (action == 'individuals.delete')
	{
		// Show confirmation message
		if (confirm('<?php echo Text::_("COM_JGIVE_DELETE_INDIVIDUAL")?>'))
		{
