...
<div class="techjoomla-bootstrap jgive" id="jgive-checkout">
	...
	name="adminForm" id="adminForm" class="form-validate form-horizontal">
		...
		<?php echo HTMLHelper::_('bootstrap.startTabSet', 'myTab', array('active' => 'info')); ?>
			...
			<?php echo HTMLHelper::_('bootstrap.addTab', 'myTab', 'info', Text::_('COM_JGIVE_ORGANIZATION_DETAILS_HEADING')); ?>
		...
		<input type="hidden" name="jform[id]" id="jform_id" value="<?php echo (!empty($this->item->id)) ? $this->item->id : 0;?>"/>
			...
			<div class="form-actions af-pt-5 af-pb-5 span9 col-md-12">
				<div class="btn-toolbar clearfix af-text-right float-end" data-js-attr="form-actions">
					<div id="toolbar-cancel" class="btn-wrapper">
						<button type="button" class="btn com_tmt_button btn-danger" onclick ="parent.jQuery('#newIndividualOrganization').modal('hide');">
							<span class="af-valign-middle"></span><?php echo Text::_('COM_JGIVE_BUTTON_CANCEL'); ?>
						</button>
					</div>&nbsp;&nbsp;

					<div id="toolbar-save" class="btn-wrapper">
						<button type="button"  class="btn btn-success com_tmt_button" onclick="Joomla.submitbutton('organization.save');"
...
<script type="text/javascript">
	/**
	var user_id = '<?php /**echo $this->item->user_id;*/ ?>';
	com_jgive.UI.Organization.hideUserCheckbox(user_id);
	*/
	Joomla.submitbutton = function(action) {
		if(action=='organization.apply' || action=='organization.save2new' || action=='organization.save') {
			...
			if (validateflag) {
					com_jgive.UI.Organization.CheckFormOnSubmit(action);
				}
			}
			else {
	...
	window.onload = (event) => {
		document.getElementById('jform_contact_name').setAttribute('autocomplete','nope');
	};
</script>
